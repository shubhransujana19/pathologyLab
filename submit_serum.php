...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "INSERT INTO serum (patient_name, uhid, ref_by, age, date, material, gender, details)
        VALUES ('$patientName', '$uhid', '$refBy', '$age', '$date', '$material', '$gender', '$details')";
            ...
            header("Location: pathology.php?table=serum&success=$successMessage");
            exit();
        } else {
            $response['status'] = 'error';
            ...
            header("Location: pathology.php?table=serum&error=$errorMessage");
            exit();
        }
...
// Output JSON response
    header('location: pathology.php?table=serum');
    exit;
