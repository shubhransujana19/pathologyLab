...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "INSERT INTO crp (patient_name, uhid, ref_by, age, date, material, gender, details)
        VALUES ('$patientName', '$uhid', '$refBy', '$age', '$date', '$material', '$gender', '$details')";
            ...
            header("Location: pathology.php?table=crp&success=$successMessage");
            exit();
        } else {
            $response['status'] = 'error';
            ...
            header("Location: pathology.php?table=crp&error=$errorMessage");
            exit();
        }
...
header('location: pathology.php?table=crp');
exit;
