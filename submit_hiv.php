...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "INSERT INTO hiv (patient_name, uhid, ref_by, age, date, material, gender, details)
        VALUES ('$patientName', '$uhid', '$refBy', '$age', '$date', '$material', '$gender', '$details')";
         ...
         if ($conn->query($sql) === TRUE) {
            ...
            header("Location: pathology.php?table=hiv&success=$successMessage");
            exit();
        } else {
            $response['status'] = 'error';
            ...
            header("Location: pathology.php?table=hiv&error=$errorMessage");
            exit();
        }
...
// Output JSON response
    header('location: pathology.php?table=hiv');
    exit;
